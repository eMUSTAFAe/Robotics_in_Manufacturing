...
class WeldingProject:
    ...
    # ==================== Task 1: 优化版TSP (多初始点+深度2-opt+全局最优) ====================
    def solve_tsp(self):
        """
        优化版TSP求解：多初始点最近邻 + 深度2-opt + 全局最优选择
        确保焊接顺序服从最短路径
        """
        print("Task 1: 正在计算最佳焊接顺序 (多初始点最近邻 + 深度2-opt)...")
        points = self.welding_points
        n = len(points)
        start_pos = self.start_point[:2]
        
        # 定义距离计算函数（包含起点的完整路径距离）
        def calculate_total_distance(order):
            """计算完整路径距离：起点 -> 焊接点序列 -> 起点"""
            if not order:
                return 0.0
            # 起点到第一个焊接点
            dist = np.linalg.norm(points[order[0]][:2] - start_pos)
            # 焊接点之间的距离
            for i in range(len(order)-1):
                dist += np.linalg.norm(points[order[i+1]][:2] - points[order[i]][:2])
            # 最后一个焊接点返回起点
            dist += np.linalg.norm(start_pos - points[order[-1]][:2])
            return dist
        
        # 步骤1：生成多个初始解（避免单一初始点的局部最优）
        initial_orders = []
        # 1.1 从起点出发的最近邻
        ...
        while remaining:
            ...
            order_nn.append(nearest)
            curr_pos = points[nearest][:2]
            remaining.remove(nearest)
        initial_orders.append(order_nn)
        
        # 1.2 随机生成多个初始解（增加多样性）
        for _ in range(5):  # 生成5个随机初始解
            rand_order = np.random.permutation(n).tolist()
            initial_orders.append(rand_order)
        
        # 步骤2：对每个初始解执行深度2-opt优化
        best_order = None
        best_dist = float('inf')
        
        for init_order in initial_orders:
            current_order = init_order.copy()
            current_dist = calculate_total_distance(current_order)
            
            # 深度2-opt迭代（增加迭代轮数，确保收敛）
            max_iterations = 1000
            iteration = 0
            improved = True
            ...
            while improved and iteration < max_iterations:
                ...
                for i in range(0, n-1):
                    for j in range(i+1, n):
                        ...
                        new_dist = calculate_total_distance(new_order)
                        
                        # 若新路径更短，更新当前解
                        if new_dist < current_dist - 1e-10:  # 浮点精度容错
        ...
        print(f"  优化后总路径长度: {best_dist:.1f}mm")
    ...
    def save_trajectory_csv(self, cart_trajs, filename="welding_trajectory_detailed.csv"):
        """将所有段的轨迹按时序写入 CSV：每行包含样本索引、段索引、末端坐标、各关节坐标和三个关节角（rad）。"""
        # 确保目录存在
        ...
        with open(filename, 'w', newline='') as f:
            ...
            sample = 0
            for seg_idx, traj in enumerate(cart_trajs):
                for q in traj:
                    if q is None:
                        # 空行或占位
                        row = [sample, seg_idx] + [''] * (len(header)-2)
                    else:
                    ...
                    writer.writerow(row)
                    sample += 1
        print(f"Saved detailed trajectory CSV: {filename}")
    ...
    def run(self):
        ...
        # 将每时刻的末端与关节信息另存为 CSV
        try:
            self.save_trajectory_csv(cart_trajs, "welding_trajectory_detailed.csv")
        except Exception as e:
            print(f"Warning: 无法保存CSV: {e}")
